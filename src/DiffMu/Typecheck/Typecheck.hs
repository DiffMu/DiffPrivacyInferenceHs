...
data VarState = Mutable | Locked
instance Show VarState where
   show Mutable = "Mutable"
   show Locked = "Locked"

type NameScope = H.HashMap Symbol VarState
instance Default NameScope where
  def = H.empty
...
checkSen' :: DMTerm -> NameScope -> TC (DMTypeOf MainKind)


checkPriv :: DMTerm -> NameScope -> TC (DMTypeOf MainKind)
...
checkSens :: DMTerm -> NameScope -> TC (DMTypeOf MainKind)
...
checkSen' (Op op args) scope =
  ...
  in do
    ...
    msumS (checkOpArg <$> (zip args arg_sens))
...
checkSen' (Lam xτs body) scope = do
     -- put all argument variable names into the scope as mutable (i.e. they can be overwritten)
     let scope' = foldl (\sc -> (\(x :- _) -> (H.insert x Mutable sc))) scope xτs

     -- check the term with the correct scope
     restype <- checkSens body scope'

     -- get the inferred sensitivities of the arguments
     xrτs <- getArgList @_ @SensitivityK xτs
     let xrτs' = [τ :@ s | (τ :@ SensitivityAnnotation s) <- xrτs]

     -- get all variables that are still in gamma -- these were captured from an outside context.
     -- we make them explicit in the -> type
     cap <- getAllVars
     let captures = [(x :- (τ :@ s)) | (x, WithRelev _ (τ :@ SensitivityAnnotation s)) <- cap]

     -- the type of this lambda
     let τ = ((captures, xrτs') :->: restype)

     -- get the type variables in the lambda type that do not appear in any constraints
     -- these are "free" for choice resolution
     frees <- getActuallyFreeVars τ

     -- get the annotated julia types
     -- TODO do we need to do anything else with these? e.g. make inferred arg types subtypes of these?
     let sign = (sndA <$> xτs)
...
checkSen' (SLet (x :- dτ) term body) scope = let
  -- check the body
  mbody sv = do
                 -- put x into the scope list as mutable
                 let scope' = H.insert x Mutable scope
                 -- check the body with x in the scope
                 τbody <- checkSens body scope'
                 -- remove x from gamma
                 WithRelev _ (τinf :@ s) <- removeVar x
                 -- make the sv variable be the inferred sensitivity so we can scale the terms gamma with that
                 unify (SensitivityAnnotation sv) s
                 return τbody
  -- check the assigned term
  mterm s = do
                 -- check the term
                 τterm <- checkSens term scope
                 -- scale with the inferred sensitivity from the body term
                 mscale s
                 return τterm
  in do
     ...
     -- overwriting locked variable names is forbidden.
     case H.lookup x scope of
            Just Locked -> throwError (ImpossibleError ("Trying to mutate locked variable " <> show x))
            _ -> return ()

     -- this will be the inferred sensitivity of x from the body, so we can scale the terms gamma
     sv <- newVar

     -- check body and term and sum the contexts
     (_, τbody) <- msumTup (mterm sv, mbody sv)
     return τbody
...
checkSen' (LamStar xτs body) scope = do
  ...
  scope' <- foldM (\sc -> (\((x :- τ), i) -> pushDefinition sc x (Var x τ i))) scope xτs

  -- TODO use correct scope...
  restype <- checkPriv body scope'
