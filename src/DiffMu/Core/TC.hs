...
instance Substitute TVarOf DMTypeOf (DMTypeOf k) where
  ...
  substitute σs (MkNum $1 MkNonConst) = NonConst <$> substitute σs τ
  substitute σs (MkNum τ (MkConst η)) = Const <$> substitute σs η <*> substitute σs τ
...
instance Typeable k => FreeVars TVarOf (DMTypeOf k) where
  ...
  freeVars (MkNum $1 MkNonConst) = freeVars τ
  freeVars (MkNum $2 (MkConst $1)) = freeVars τ
