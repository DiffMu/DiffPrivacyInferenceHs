...
instance Substitute TVarOf DMTypeOf (DMTypeOf k) where
  ...
  substitute σs (DMVecLike k nrm clp n τ) = DMVecLike <$> substitute σs k <*> substitute σs nrm <*> substitute σs clp <*> substitute σs n <*> substitute σs τ
...
instance Substitute SVarOf SensitivityOf (DMTypeOf k) where
  ...
  substitute σs (τ1 :->*: τ2) = (:->*:) <$> substitute σs τ1 <*> substitute σs τ2
  substitute σs (DMTup τs) = DMTup <$> substitute σs τs
  substitute σs (DMVecLike k nrm clp n τ) = DMVecLike k nrm clp <$> substitute σs n <*> substitute σs τ
...
instance Typeable k => FreeVars TVarOf (DMTypeOf k) where
  ...
  freeVars (DMVecLike k nrm clp n τ) = freeVars k <> freeVars nrm <> freeVars clp <> freeVars τ
