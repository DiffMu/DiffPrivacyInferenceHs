...
module DiffMu.Core.SimpleScope where
...
-- instance Default SimpleScope where
--   def = DMScope def



-- instance Show SimpleScope where
--   show scope = showWith "\n" f (mergeScopes scope) ""
...
-- popDefinition :: MonadDMTC t => SimpleScope -> Symbol -> t (DMTerm, SimpleScope)
-- popDefinition (varscope) v = do
--    case H.lookup v varscope of
--       Just (vvarscope)  -> return (vvarscope)
--       Nothing -> throwError (VariableNotInScope v)

  {-
-- Given a scope, a variable name v , and a DMTerm t, we push t to the list for v.
pushDefinition :: (MonadDMTC t) => SimpleScope -> Symbol -> DMTerm -> t SimpleScope
...
pushChoice :: (MonadDMTC t) => SimpleScope -> Symbol -> [JuliaType] -> DMTerm -> t SimpleScope
